...
class Classifier:
    ...
    def __read_training_dataset(self):
        ...
        # # ICDAR Male training data acquisition
        # for file_name in sorted(glob.glob('../Training Dataset/icdar/males/*.jpg')):
        #     # img = cv2.imread(file_name)      # cv2.imread reads images in RGB format
        #     x_train.append(file_name)
        #     y_train.append(1)

        # # ICDAR Female training data acquisition
        # for file_name in sorted(glob.glob('../Training Dataset/icdar/females/*.jpg')):
        #     # img = cv2.imread(file_name)      # cv2.imread reads images in RGB format
        #     x_train.append(file_name)
        #     y_train.append(0)
    ...
    def __train_model(self):
        ...
        # combine
        self.__combined = EnsembleVoteClassifier(self.__classifiers)
        self.__combined.fit(self.__X_train_std, self.__Y_train)
    ...
    def classify(self, feature_vector=None):
        if self.__SPLIT_TRAINING_DATA:
            ...
            Y_predicted = self.__combined.predict(X_test_std)

            ###
            # i = 0
            # for cls in self.__classifiers:
            #     cls.fit(self.__X_train_std, self.__Y_train)
            #     y = cls.predict(X_test_std)
            #     print('cls', i, metrics.accuracy_score(self.__Y_test, y))
            #     i+=1
            ###
